 ...
 *
 **    o 최종 수정: (2021-04-04): 개행문자가 'n'인 경우 제대로 개행문자 제거 안되는 문제를 해결함                                            
 ** 
 **    o 참고사항
...
public class SkScanner { 
	...
	// removeLastNewLineChar(): 정수, 실수, 문자열, 식별자 등을 읽기 위해 입력된 개행문자('\r', '\n')를 제거시킴
	//   정수, 실수, 문자열, 식별자 등을 읽기 위해 입력된 마지막 개행문자를 제거시켜 
	//   다음에 getChar()를 했을 때 개행문자가 또다시 읽혀지지 않게 함
	//   이 메소드는 반드시 getLong(), getDouble(), getString(),  getIdentifier()메소드의 마지막에서 호출되어야 함
            //
	//  (2021-04-04 수정): 개행문자가 'n'인 경우 제대로 개행문자 제거 안되는 문제 해결함 
	static void removeLastNewLineChar() {
		int c1, c2;
		if ( (c1 = getcharPrivate()) == '\r') {
			if ( (c2 = getcharPrivate()) == '\n')		
				return;  // 개행문자가 '\r', '\n'인 경우(윈도즈 시스템의 개행문자) 이를 읽어 제거시킴 
			else {
				ungetc(c2);
				ungetc(c1);
			}
		} 
		else if (c1 == '\n')   // 2021-04-04 수정: 개행문자가 '\n'인 경우(유닉스와 리눅스의 개행문자)에 대해 '\n'을 제거함
			return;
		else
			ungetc(c1);  // 개행문자 아니면 읽은 문자를 다시 읽도록 ungetc
	}
...
//		System.out.println("  o head of identifier = '" + (char) head + "' (" + head + ")"); 
   ...
   public static boolean isAlphabet(int c) {
		...
		// 유니코드에서 한글 문자는 '가'('\uAC00': 44032)에서 '?R'('\uD7A3': 55203)까지 배정됨
