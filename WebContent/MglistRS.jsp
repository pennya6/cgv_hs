...
System.out.print("   >> clms : " + columns[i] + " " + columnTypes[i]+ "\n");	
	  ...
	  %>
       <%
	    ...
	    out.print("<table align=center valign=top border=1 cellpadding=8 cellspacing=0 bordercolor=#999999>");
			...
			if(title.equals("모든 리뷰 조회")||title.equals("모든 영화 조회") || title.equals("모든 고객 조회")||title.equals("모든 상영 조회")||title.equals("모든 극장 조회")||title.equals("모든 상영관 조회")) {
			out.print("<th>삭제</th>" );	
			out.print("</tr>" );}
 ...
 // System.out.print("   >> col value : " + (obj) + "\n");
				...
				%> 
				<%if(title.equals("모든 상영 조회")||title.equals("모든 리뷰 조회")||title.equals("모든 영화 조회") || title.equals("모든 고객 조회")||title.equals("모든 극장 조회")||title.equals("모든 상영관 조회")) {
				out.print("<td>" );
				col_0[n]= rs.getObject(columns[0]); //memberno,movieno,theatername,screentheaternames%>
				<form action="Mgdelete.jsp" method="post">
...
%>
	  ...
	  <%if(title.equals("모든 리뷰 조회")||title.equals("모든 영화 조회") || title.equals("모든 고객 조회")||title.equals("모든 상영 조회")||title.equals("모든 극장 조회")||title.equals("모든 상영관 조회")) {%>
	    ...
	    <input type="submit" id="valueId" VALUE="삭제" onClick="delete_click();")/><%} %>
	  
	  </form>
	  <%
	  //Mg~Add.jsp로 넘기기
        if(title.equals("모든 영화 조회"))
         %> <button type="button" onclick="location.href='MgMovieAdd.jsp' ">추가</button><% 
       else if(title.equals("모든 상영관 조회"))
        %> <button type="button" onclick="location.href='MgScreenTheaterAdd.jsp' ">추가</button><%
       else if(title.equals("모든 상영 조회"))
           %> <button type="button" onclick="location.href='MgScreenAdd.jsp' ">추가</button> 
