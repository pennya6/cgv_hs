...
<body>
	...
	<div id="showContent">
		...
		<div><form action="InsertScreenTheater.jsp" method=post> 
		<%  
		
		     DB.loadConnectCGVhs();
		     ResultSet rs = DB.getAlltheaterRS(); %>
		    상영관 이름  <input type="text" name="stname" size=15> <br> <br>
		       좌석수  <input type="text" name="seats" size=15> <br> <br>
	             영화관 선택  <select name="t_name">
	             <%
	                  String[] t_name=new String[100];
	                  int i=0;
	                  while(rs.next()){
	                	  theater t = DB.gettheaterFromRS(rs);
	                	  t_name[i]=t.gettheatername();
	                	  i++;
	                  }
	             
	                  for(int k=0;k<i;k++){
	             %>
	             	<option value="<%= t_name[k]%>"><%= t_name[k] %></option><%} %>
      			</select>
